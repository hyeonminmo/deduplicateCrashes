...
TARGETPATH = {
    ...
    'libjpeg-turbo': os.path.join(FTSPATH, 'libjpeg-turbo/libjpeg-turbo'),
    ...
    'libxml2': os.path.join(FTSPATH, 'libxml2/libxml2'),
    ...
    'wpantund': os.path.join(FTSPATH, 'wpantund/wpantund'),
...
FRONTOPTIONS = {
    ...
    'mp42aac': ' ', #
    'mujs': ' ', #
    'nm': ' -A -a -l -S -s --special-syms --synthetic --with-symbol-versions -D ', #
    'objdump': ' -S ', #
    'pdftotext': ' ', #
    # 'readelf': '',
    # 'size': '',
...
REAROPTIONS = {
    ...
    # 'cxxfilt': '',
    'exiv2': '',
    'ffmpeg': ' -c:v mpeg4 -c:a copy -f mp4 /dev/null',
    'flvmeta': '',
    'imginfo': '',
    'infotocap': '',
    'jhead': '',
                    ...
                    for file in files_:
                        if 'input' in file:
                            ...
                            if filePath not in crashes:
                                command = "ASAN_OPTIONS=\"log_path=" + os.path.join(saveDir, str(trial) + str(n)) + "\" " + targetProgramPath + FRONTOPTIONS[targetProgram] + filePath + REAROPTIONS[targetProgram] + "\n"
                                n += 1
                                f.write(command)
